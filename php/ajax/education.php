...
if (!empty($request['search']['value'])) {
	$sql = $sql." AND (`product`.`id` LIKE '%$srch_key%' OR `product`.`name` LIKE '%$srch_key%' OR `category`.`name`  LIKE '%$srch_key%' OR `sub_category`.`name` LIKE '%$srch_key%' OR `product`.`price` LIKE '%$srch_key%' OR `product`.`stock` LIKE '%$srch_key%')";
}
	// Ordering
...
if ($res = $connection->query($sql)) {
	...
	while ($row = $res->fetch_assoc()) {
		...
		if ($row['is_tranding'] == '1') {
			$action .= '<a href="php/product/make_trending_product.php?p_id='.$row['id'].'&page=1005" class="btn btn-primary">Make Tranding Product</a>';
		}
		if ($row['is_popular'] == '1') {
			$action .= '<a href="php/product/make_popular_product.php?p_id='.$row['id'].'&page=1005" class="btn btn-primary">Make Popular Product</a>';
		}
		$action .= '<a href="product_edit.php?p_id='.$row['id'].'&page=1005" class="btn btn-success">Edit</a>
			...
			<a href="add_related_product_form.php?p_id='.$row['id'].'&page=1005" class="btn btn-warning">Add Related Product</a>';
		$data[] = [
			$count,
			$row['id'],
			$row['name'],
