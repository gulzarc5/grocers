...
if(isset($_POST['update_product']) && !empty($_POST['p_id'])){
   ...
   if ($res_p_check = $connection->query($sql_p_check)) {
    ...
    $image_name = null;
        ...
        $sql_update_product = "UPDATE `product` SET `brand_id`='$brand',`name`='$name',`mrp`='$mrp',`category_id`='$category',`sub_cat_id`='$sub_category',`description`='$description',`price`='$price', `stock`='$stock',`is_tranding`='$tranding',`is_popular`='$popular', `expiry_date` = '$expiry_date' WHERE `id`='$p_id'";
